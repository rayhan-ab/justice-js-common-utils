...
export class PermissionGuard {
  ...
  public getCrudTypeByCrudBit = (action: number) => {
    const actionBits = Object.keys(CrudBitType)
      .map(Number)
      .filter(Boolean);
  ...
  private checkHasPermission = (
    ...
    { resource, action }: CrudRolePermission
  ): boolean => {
    return (
      Array.isArray(userPermissions) &&
      userPermissions.some((permission: RolePermission) => {
        return (
          !!permission && this.isResourceIncluded(permission, resource) && this.isActionIncluded(permission, action)
        );
      })
    );
  };
  ...
  private isResourceIncluded = (permission: RolePermission, resource: string): boolean => {
    ...
    const maxSectionLength =
      requiredResourceArr.length > testResourceArr.length ? testResourceArr.length : requiredResourceArr.length;
  ...
  private replaceResourceVal = (value: string, replacement: { [key: string]: string }) => {
    return Object.keys(replacement).reduce(
      (val: string, key: string) => val.replace(`{${key}}`, replacement[key]),
      value
    );
  };
}
